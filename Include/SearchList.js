...
// *************************************************************************************************************
    ...
    function ComboBox_TextTypeDown(e) { //Make the textbox do a type-down on the list
        ...
        var ctrlKeys = Array( 46, 37, 38, 39, 40, 33, 34, 35, 36, 45, 16, 20 );
...
// *************************************************************************************************************
		...
		{
		    var i, j;
			var gridId = "dg1" //ctrl.parentElement.parentElement.parentElement.parentElement.id;
			var rowCount = document.getElementById("Hid_RowCount").value ;
			var colCount = document.getElementById("Hid_ColCount").value ;
			for(i=0; i<rowCount; i++)
		    {
		        var ctrlItem = document.getElementById(gridId+"_itm"+i);
		        var chkBox = document.getElementById(gridId+"_itm"+i+"_chkSelect");
		        //chkBox.checked = false;
		        chkBox.src = "../Images/images3.JPG";
		        chkBox.parentElement.parentElement.style.backgroundColor='#EAF7FF'; 
		        chkBox.parentElement.parentElement.style.color='black';
		        ctrlItem.style.display = "block";
		        for(j=0; j<colCount-1; j++)
		        {
		            var TextBox = document.getElementById(gridId+"_itmCombos_ComboBox"+j+"_Text");
		            var TextValue = TextBox.value.toLowerCase();
		            var TextLen = TextValue.length 
		            var value = ctrlItem.children(j+1).innerHTML.toLowerCase();
		            value = value.substring(0,TextLen);
		            //alert("Value = " + value + " TextValue = " + TextValue);
	                if ((value==TextValue || TextValue=="all")==false)
                    {   
                        ctrlItem.style.display = "none";
                        break;
                    }
		        }
		    }
		...
		function CheckOption(ctrlChk)
	    {
	        UncheckAll(ctrlChk); 
	        //ctrlChk.checked = true;
	        ctrlChk.src = "../Images/images.JPG";
            ctrlChk.parentElement.parentElement.style.backgroundColor='#D1E4F8';  
		    ctrlChk.parentElement.parentElement.style.color='black'; 
        }
        function UncheckAll(ctrl)
        {
            var gridId = 'dg1' //ctrl.parentElement.parentElement.parentElement.parentElement.id;
		    var rowCount =  document.getElementById(gridId).rows.length-3 ;
		    //document.getElementById("Hid_RowCount").value ;
		    for (i=0; i<rowCount; i++)
			{   
		       var chkBox = document.getElementById(gridId+"_itm"+i+"_chkSelect");
		       chkBox.src = "../Images/images3.JPG";
        ...
        function Sumbit()
        {
            ...
            for (i=0; i<rowCount; i++)
			{
			    ...
			    var chkBox = document.getElementById(gridId+"_itm"+i+"_chkSelect");
			    //alert(chkBox.src);
			    var bgColor = ctrlItem.style.backgroundColor.toUpperCase();
			    if(bgColor=='#D1E4F8')
			    {
			        for(j=0; j<colCount-2; j++)
			        {   
			            retString = retString + ctrlItem.children(j+1).children(0).innerHTML + "!";
			        }
			        var HiddenValue = document.getElementById("Hid_RetValues").value.split("!");
//			        alert(HiddenValue[i])
			        retString = retString + HiddenValue[i] + "!"; 
