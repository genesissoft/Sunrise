...
Partial Class Forms_StockValuation
    ...
    Dim objMISRpt As New MISReports
    ...
    Private Sub FillYieldOptions()
        Try
            ...
            ElseIf decRate = 0.0 Then
                If strSemiAnnFlag = "Y" Or strSemiAnnFlag = "A" Or strSemiAnnFlag = "N" Then
                    Hid_YTMAnn.Value = decYield
                    strSemiAnnFlag = "A"
                ElseIf strSemiAnnFlag = "H" Or strSemiAnnFlag = "S" Then
                    Hid_YTMSemi.Value = IIf(Not blnNonGovernment, decYield, decYTMSemi)
                    strSemiAnnFlag = "S"
                End If
                ...
                GlobalFuns.CalculateYield(datYTM, decFaceValue, decRate, blnNonGovernment, blnRateActual, datMaturity, decMaturityAmt,
    ...
    Private Sub FillXIRROptions()
        Try
            ...
            ElseIf decYield <> 0.0 Or decYTMSemi <> 0.0 Then
                ...
                CalculateXIRRPrice(SecurityId, txt_ForDate.Text, Val(Hid_Frequency.Value), False, strSemiAnnFlag, "R", "N", "M")
            ...
            ElseIf decYTC <> 0.0 Or decYTCSem <> 0.0 Then
                ...
                CalculateXIRRPrice(SecurityId, txt_ForDate.Text, Val(Hid_Frequency.Value), False, strSemiAnnFlag, "R", "N", "C")
                'CalculateXIRRPrice(SecurityId, txt_ForDate.Text, Val(Hid_Frequency.Value), False, strSemiAnnFlag, "R", "C")
                Hid_Rate.Value = dblPTC
            Else
                CalculateXIRRYield(SecurityId, txt_ForDate.Text, Val(decRate), Val(Hid_Frequency.Value), False, "R", "N")
    ...
    Protected Sub btn_Export_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btn_Export.Click
        Try
            dtRpt = ViewReport("ID_Fill_SecurityEvaluation")
            If dtRpt IsNot Nothing And dtRpt.Rows.Count > 0 Then
                objMISRpt.ExportToExcel_SecurityEvaluation(dtRpt, txt_ForDate.Text)
            Else
    ...
    Private Function ViewReport(ByVal strProcName As String) As DataTable
        Try
            Dim strcon As String = ""
            Dim sqlda As New SqlDataAdapter
            Dim sqldt As New DataTable
            ...
            Dim sqldv As New DataView
            Dim dateFor As Date
            dateFor = objCommon.DateFormat(txt_ForDate.Text)
            OpenConn()
            sqlComm.Connection = sqlConn
            sqlComm.CommandTimeout = "100"
            sqlComm.CommandType = CommandType.StoredProcedure
            ...
            sqlComm.Parameters.Clear()
            objCommon.SetCommandParameters(sqlComm, "@ForDate", SqlDbType.SmallDateTime, 8, "I", , , dateFor)
            objCommon.SetCommandParameters(sqlComm, "@compid", SqlDbType.Int, 4, "I", , , Val(Session("CompId")))
