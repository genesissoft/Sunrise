...
Imports log4net
Partial Class Forms_IssuerMaster_RDM
    ...
    Private Sub SetAttributes()
        Try
            ...
            OpenConn()
            objCommon.FillControl(cbo_IssuerType, sqlConn, "ID_FILL_IssuerTypeMaster_RDM", "IssuerTypeName", "RDMIssuerTypeid")
    ...
    Private Sub FillFields()
        Try
            OpenConn()
            Dim dt As DataTable
            'objCommon.FillControl(cbo_IssuerType, sqlConn, "ID_FILL_IssuerMaster_RDM", "IssuerTypeName", "RDMIssuerTypeid")
            dt = objCommon.FillDataTable1(sqlConn, "ID_FILL_IssuerMaster_RDM", ViewState("Id"), "RDMIssuerId")
            If dt.Rows.Count > 0 Then
                cbo_IssuerType.SelectedValue = Val(dt.Rows(0).Item("RDMIssuerTypeid") & "")
                txt_IssuerName.Text = Trim(dt.Rows(0).Item("IssuerName") & "")

                rdo_HideShow.selectedvalue = Trim(dt.Rows(0).Item("Hide") & "")
            End If
        Catch ex As Exception
            ScriptManager.RegisterStartupScript(Me.Page, Me.GetType(), "msg", "alert('" & ex.Message.Replace("'", " ").Replace(Chr(13), " ").Replace(Chr(10), " ") & "')", True)
        Finally
    ...
    Private Sub SetSaveUpdate(ByVal strProc As String)
        Try
            ...
            sqlTrans.Commit()
            Response.Redirect("IssuerMasterDetail_RDM.aspx?Id=" & HttpUtility.UrlEncode(objCommon.EncryptText(ViewState("Id"))), False)
        Catch ex As Exception
            ScriptManager.RegisterStartupScript(Me.Page, Me.GetType(), "msg", "alert('" & ex.Message.Replace("'", " ").Replace(Chr(13), " ").Replace(Chr(10), " ") & "');", True)
        Finally
            CloseConn()
    ...
    Private Function SaveUpdate(ByVal sqlTrans As SqlTransaction, ByVal strProc As String) As Boolean
        Try
            ...
            objCommon.SetCommandParameters(sqlComm, "@RDMIssuerTypeid", SqlDbType.Int, 4, "I", , , Val(cbo_IssuerType.SelectedValue))
            objCommon.SetCommandParameters(sqlComm, "@Hide", SqlDbType.Char, 1, "I", , , Trim(rdo_HideShow.SelectedValue))
    ...
    Protected Sub btn_Back_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btn_Back.Click
        If Val(ViewState("Id")) <> 0 Then
            Response.Redirect("IssuerMasterDetail_RDM.aspx?Id=" & HttpUtility.UrlEncode(objCommon.EncryptText(ViewState("Id"))), False)
        Else
            Response.Redirect("IssuerMasterDetail_RDM.aspx")
        End If
